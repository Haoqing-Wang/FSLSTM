...
if __name__ == '__main__':
    ...
    # If you do not use the pre-training initialization, comment out the following code and take opt.lr=0.1
    ...
    optimizer = torch.optim.SGD([{'params':embedding_net.parameters(), 'lr':opt.lr}, {'params': cls_head.parameters()}], lr=0.1, momentum=0.9, weight_decay=5e-4, nesterov=True)
    ...
    for epoch in range(1, opt.num_epoch+1):
        ...
        if val_acc_avg > max_val_acc:
            ...
            torch.save({'embedding': embedding_net.state_dict(), 'head': cls_head.state_dict()}, os.path.join(opt.save_path, 'best_model.pth'))
        ...
        torch.save({'embedding': embedding_net.state_dict(), 'head': cls_head.state_dict()}, os.path.join(opt.save_path, 'last_epoch.pth'))
        if epoch % opt.save_epoch == 0:
            torch.save({'embedding': embedding_net.state_dict(), 'head': cls_head.state_dict()}, os.path.join(opt.save_path, 'epoch_{}.pth'.format(epoch)))
