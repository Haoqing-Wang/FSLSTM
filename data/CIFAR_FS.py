...
CIFAR_FS_DATASET_DIR = 'Datasets/CIFAR_FS'

def buildLabelIndex(labels):# return dict: key=label, value=Index
    label2inds = {}
...
def load_data(file): #Load dataset
    try:
...
class CIFAR_FS(data.Dataset):
    def __init__(self, args, phase='train', do_not_use_random_transf=False):
        assert(phase=='train' or phase=='val' or phase=='test')
        self.phase = phase
        self.name = 'CIFAR_FS_' + phase
        print('Loading CIFAR-FS dataset - phase {0}.'.format(phase))
        file_train_categories_train_phase = os.path.join(CIFAR_FS_DATASET_DIR,'CIFAR_FS_train.pickle')
        file_train_categories_val_phase = os.path.join(CIFAR_FS_DATASET_DIR,'CIFAR_FS_train.pickle')
        file_train_categories_test_phase = os.path.join(CIFAR_FS_DATASET_DIR,'CIFAR_FS_train.pickle')
        file_val_categories_val_phase = os.path.join(CIFAR_FS_DATASET_DIR,'CIFAR_FS_val.pickle')
        file_test_categories_test_phase = os.path.join(CIFAR_FS_DATASET_DIR,'CIFAR_FS_test.pickle')
        ...
        if self.phase=='train':
            ...
            self.data = data_train['data'] #(N_images, height, width, n_channel)
            self.labels = data_train['labels'] #(N_images,)
            self.label2ind = buildLabelIndex(self.labels)
            self.labelIds = sorted(self.label2ind.keys()) #categories
            self.num_cats = len(self.labelIds) #number of categories
            self.labelIds_base = self.labelIds #base categories
            self.num_cats_base = len(self.labelIds_base) #number of base categories
            ...
            self.num_cats = len(self.labelIds) #number of total categories
            self.labelIds_base = buildLabelIndex(data_base['labels']).keys() #base categories
            self.labelIds_novel = buildLabelIndex(data_novel['labels']).keys() #novel categories
            self.num_cats_base = len(self.labelIds_base) #number of base categories
            self.num_cats_novel = len(self.labelIds_novel) #number of novel categories
        ...
        # Transformation
        if args.network == 'Conv4':
            mean_pix = [0.485, 0.456, 0.406]
            std_pix = [0.229, 0.224, 0.225]
        elif args.network == 'ResNet12':
        ...
        if (self.phase=='test' or self.phase=='val') or (do_not_use_random_transf==True):
            ...
            self.transform = transforms.Compose([
                transforms.RandomCrop(32, padding=4), transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
                transforms.RandomHorizontalFlip(), lambda x: np.asarray(x), transforms.ToTensor(), normalize])
...
class FewShotDataloader():
    def __init__(self,
        ...
        max_possible_nKnovel = (self.dataset.num_cats_base if self.phase=='train' else self.dataset.num_cats_novel)
    ...
    def sampleImageIdsFrom(self, cat_id, sample_size=1):
        """
        Samples `sample_size` number of unique image ids picked from the category `cat_id` (i.e., self.dataset.label2ind[cat_id]).
        Args:
            cat_id: a scalar with the id of the category from which images will be sampled.
    ...
    def sampleCategories(self, cat_set, sample_size=1):
        """
        Samples `sample_size` number of unique categories picked from the `cat_set` set of categories. `cat_set` can be either 'base' or 'novel'.
        Args:
    ...
    def sample_base_and_novel_categories(self, nKbase, nKnovel):
        ...
        Returns:
            Kbase: a list of length 'nKbase' with the ids of the sampled base categories.
            Knovel: a list of lenght 'nKnovel' with the ids of the sampled novel categories.
        """
    ...
    def sample_test_examples_for_base_categories(self, Kbase, nTestBase):
        ...
        Args:
            Kbase: a list of length `nKbase` with the ids of the categories from where the images will be sampled.
            nTestBase: the total number of images that will be sampled.
        Returns:
            Tbase: a list of length `nTestBase` with 2-element tuples. The 1st element of each tuple is the image id that was sampled and the
                2nd elemend is its category label (which is in the range [0, len(Kbase)-1]).
        """
        Tbase = []
        if len(Kbase) > 0:
            # Sample for each base category a number images such that the total number sampled images of all categories to be equal to `nTestBase`.
    ...
    def sample_train_and_test_examples_for_novel_categories(self, Knovel, nTestNovel, nExemplars, nKbase):
        """Samples train and test examples of the novel categories.
        Args:
    	    Knovel: a list with the ids of the novel categories.
            nTestNovel: the total number of test images that will be sampled from all the novel categories.
            nExemplars: the number of training examples per novel category that will be sampled.
            nKbase: the number of base categories. It is used as offset of the category index of each sampled image.
        Returns:
            Tnovel: a list of length `nTestNovel` with 2-element tuples. The 1st element of each tuple is the image id that was sampled and
                the 2nd element is its category label (which is in the range [nKbase, nKbase + len(Knovel) - 1]).
            Exemplars: a list of length len(Knovel) * nExemplars of 2-element tuples. The 1st element of each tuple is the image id that was
                sampled and the 2nd element is its category label (which is in the range [nKbase, nKbase + len(Knovel) - 1]).
        """
        if len(Knovel) == 0:
            return [], []
    ...
    def sample_episode(self):
        ...
        nKnovel = self.nKnovel  #number of novel categories
        nKbase = self.nKbase  #number of base categories
        nTestNovel = self.nTestNovel  #the total number of test images that will be sampled from all the novel categories.
        nTestBase = self.nTestBase  #the total number of test images that will be sampled from all the base categories.
        nExemplars = self.nExemplars  #the number of training examples per novel category that will be sampled.

        Kbase, Knovel = self.sample_base_and_novel_categories(nKbase, nKnovel) #sampled base categories and sampled novel categories.
        Tbase = self.sample_test_examples_for_base_categories(Kbase, nTestBase)  #test examples for base categories
        Tnovel, Exemplars = self.sample_train_and_test_examples_for_novel_categories(Knovel, nTestNovel, nExemplars, nKbase) #test and train examples for novel categories
    ...
    def createExamplesTensorData(self, examples):
        ...
        Args:
            examples: a list of 2-element tuples, each representing a train or test example. The 1st element of each tuple
                is the image id of the example and 2nd element is the category label of the example, which is in the range
                [0, nK - 1], where nK is the total number of categories (both novel and base).
        Returns:
            images: a tensor of shape [nExamples, Height, Width, 3] with the example images, where nExamples is the number of examples (i.e., nExamples = len(examples)).
            labels: a tensor of shape [nExamples] with the category label of each example.
        """
    ...
    def get_iterator(self, epoch=1):
        ...
        data_loader = tnt_dataset.parallel(batch_size=self.batch_size, num_workers=(0 if self.is_eval_mode else self.num_workers), shuffle=(False if self.is_eval_mode else True))
