...
MINI_IMAGENET_DATASET_DIR = 'Datasets/MiniImageNet'
...
class MiniImageNet(data.Dataset):
    def __init__(self, args, phase='train', do_not_use_random_transf=False):
        ...
        file_train_categories_train_phase = os.path.join(MINI_IMAGENET_DATASET_DIR,'miniImageNet_category_split_train_phase_train.pickle')
        file_train_categories_val_phase = os.path.join(MINI_IMAGENET_DATASET_DIR,'miniImageNet_category_split_train_phase_val.pickle')
        file_train_categories_test_phase = os.path.join(MINI_IMAGENET_DATASET_DIR, 'miniImageNet_category_split_train_phase_test.pickle')
        file_val_categories_val_phase = os.path.join(MINI_IMAGENET_DATASET_DIR, 'miniImageNet_category_split_val.pickle')
        file_test_categories_test_phase = os.path.join(MINI_IMAGENET_DATASET_DIR, 'miniImageNet_category_split_test.pickle')
        ...
        if (self.phase=='test' or self.phase=='val') or (do_not_use_random_transf==True):
            ...
            self.transform = transforms.Compose([transforms.RandomCrop(84, padding=8), transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
...
class FewShotDataloader():
    ...
    def sampleCategories(self, cat_set, sample_size=1):
        ...
        Args:
            cat_set: string that specifies the set of categories from which categories will be sampled.
    ...
    def sample_train_and_test_examples_for_novel_categories(self, Knovel, nTestNovel, nExemplars, nKbase):
        ...
        Returns:
            ...
            Exemplars: a list of length len(Knovel) * nExemplars of 2-element tuples. The 1st element of each tuple is the image id that was
                sampled and the 2nd element is its category label (which is in the ragne [nKbase, nKbase + len(Knovel) - 1]).
        """
        if len(Knovel) == 0:
            return [], []
    ...
    def sample_episode(self):
        ...
        nTestNovel = self.nTestNovel  #number of test examples for all the novel categories.
        nTestBase = self.nTestBase  #number of test examples for all the base categories.
        nExemplars = self.nExemplars  #number of training examples per novel category.
